...
int main(int argc, char* argv[])
{
  ...
  initial_values.insert(E(correction_count), prior_pose);  
  ...
  gravity << 0, 0, 0;
  p->n_gravity = gravity;
  imu_preintegrated_ = new RelativePreintegratedImuMeasurements(p, prior_imu_bias);
  ...
  while (file.good()) {
    ...
    if (type == 0) { // IMU measurement
      ...
      RelativePreintegratedImuMeasurements *preint_imu = dynamic_cast<RelativePreintegratedImuMeasurements*>(imu_preintegrated_);
      RelativeImuFactor imu_factor(X(correction_count-1), V(correction_count-1),
                           X(correction_count  ), V(correction_count  ), E(0), 
      ...
      cout << imu_preintegrated_->p().n_gravity << endl;   
      prop_state = imu_preintegrated_->predict(prev_state, prev_bias);
      initial_values.insert(X(correction_count), prop_state.pose());
      initial_values.insert(V(correction_count), prop_state.v());
      initial_values.insert(B(correction_count), prev_bias);
      //initial_values.insert(E(0), prior_pose);
